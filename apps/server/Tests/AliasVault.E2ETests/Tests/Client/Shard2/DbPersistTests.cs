...
public class DbPersistTests : ClientPlaywrightTest
{
    ...
    public async Task VaultRevisionNumberIncrementTest()
    {
        ...
        var firstUpdateVault = await ApiDbContext.VaultManifests.AsNoTracking().OrderByDescending(x => x.RevisionNumber).FirstAsync();
        ...
        var secondUpdateVault = await ApiDbContext.VaultManifests.AsNoTracking().OrderByDescending(x => x.RevisionNumber).FirstAsync();
