...
public class ResetVaultTests : ClientPlaywrightTest
{
    ...
    public async Task ResetVaultTest()
    {
        ...
        var latestVault = await ApiDbContext.VaultManifests.AsNoTracking().Where(x => ApiDbContext.AliasVaultUsers.Any(u => u.UserName == TestUserUsername && u.PersonalGroupId == x.OwnerGroupId)).OrderByDescending(x => x.CreatedAt).FirstAsync();
        ...
        latestVault = await ApiDbContext.VaultManifests.AsNoTracking().Where(x => ApiDbContext.AliasVaultUsers.Any(u => u.UserName == TestUserUsername && u.PersonalGroupId == x.OwnerGroupId)).OrderByDescending(x => x.CreatedAt).FirstAsync();
        Assert.Multiple(() =>
        {
