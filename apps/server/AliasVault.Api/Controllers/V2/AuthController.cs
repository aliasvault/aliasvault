        ...
        if (result.Succeeded)
        {
            ...
            await using (var context = await dbContextFactory.CreateDbContextAsync())
            {
                ...
                context.VaultManifests.Add(new AliasServerDb.VaultManifest
                {
                    ...
                    Version = "0.0.0",
                    RevisionNumber = 0,
                    Salt = createsVaultKey ? null : model.Salt,
                    Verifier = createsVaultKey ? null : model.Verifier,
                    EncryptionType = createsVaultKey ? null : model.EncryptionType,
                    EncryptionSettings = createsVaultKey ? null : model.EncryptionSettings,
                    FileSize = 0,
                    CreatedAt = timeProvider.UtcNow,
                    UpdatedAt = timeProvider.UtcNow,
                });
