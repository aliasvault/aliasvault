...
public class AuthController(IAliasServerDbContextFactory dbContextFactory, UserManager<AliasVaultUser> userManager, IConfiguration configuration, IMemoryCache cache, ITimeProvider timeProvider, AuthLoggingService authLoggingService, Config config, ServerSettingsService settingsService) : ControllerBase
{
    ...
    /// <summary>
    /// Register endpoint (v1). Account creation via the v1 API is no longer supported: starting from
    /// this release only v2-capable clients are allowed to create accounts. The account-creation logic
    /// has been intentionally removed so that outdated clients (and old tests) can no longer register via v1.
    /// </summary>
    /// <returns>IActionResult.</returns>
    [HttpPost("register")]
    public IActionResult Register()
    {
        // v1 registration is intentionally disabled. Clients must use the v2 API to create accounts.
        return StatusCode(410, ApiErrorCodeHelper.CreateValidationErrorResponse(ApiErrorCode.REGISTRATION_FAILED, 410));
    }
