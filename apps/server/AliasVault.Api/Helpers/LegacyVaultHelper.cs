...
public static class LegacyVaultHelper
{
    ...
    {
        return await context.VaultManifests.AnyAsync(x => x.StorageFormat == VaultManifestBase.ManifestStorageFormat && context.AliasVaultUsers.Any(u => u.Id == userId && u.PersonalGroupId == x.OwnerGroupId))
