...
import type { PasswordChangeInitiateResponse, PasswordChangeRequest } from '@/utils/dist/core/models/webapi';
import { EncryptionUtility } from '@/utils/EncryptionUtility';
import { getStorageItem } from '@/utils/StorageUtility';
import { VaultKeyService } from '@/utils/VaultKeyService';
import { VaultSyncHoldReason, withVaultSyncHold } from '@/utils/VaultSyncHold';
...
export class MasterPasswordService {
  ...
  public static async getStoredDerivationParams(): Promise<EncryptionKeyDerivationParams | null> {
    return getStorageItem<EncryptionKeyDerivationParams>(StorageKeys.ENCRYPTION_KEY_DERIVATION_PARAMS);
  }
  ...
  public static async answerSrpChallenge(challenge: SrpChallenge, password: string): Promise<SrpChallengeAnswer> {
     ...
     */
    const username = (await storage.getItem(StorageKeys.USERNAME)) as string | null;
