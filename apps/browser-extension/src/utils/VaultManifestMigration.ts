 ...
 * 2. A missing vault key ({@link requiresLegacyVaultKeyMigration}), one-time and deletable — see that file.
 *
...
export async function vaultRequiresManifestMigration(sqliteClient: SqliteClient): Promise<boolean> {
  return await sqliteClient.requiresSchemaMigration() || await requiresLegacyVaultKeyMigration();
}
