...
class VaultSync(
    ...
    private fun syncError(result: JSONObject): AppError {
        ...
        return when (result.optString("errorCode")) {
            ...
            "E-203" -> AppError.VaultDecryptFailed()
            "E-206" -> AppError.UnlockKeyRejected()
            "E-207" -> AppError.KeyChainUnreadable(message)
            "E-208" -> AppError.KeyOutOfSync()
            "E-503" -> AppError.ServerVaultDecryptFailed(message)
