...
import type { BucketRevision, ManifestRevision } from '@aliasvault/models/webapi';
...
export async function replaceManifestRevisions(revisions: Record<string, number>): Promise<void> {
  await getPlatform().storage.set(StorageKeys.SERVER_MANIFEST_REVISIONS, revisions);
}
...
export async function getBucketRevisions(): Promise<Record<string, number>> {
  return ((await getPlatform().storage.get(StorageKeys.VAULT_BUCKET_REVISIONS)) as Record<string, number> | null) ?? {};
}
...
export async function replaceBucketRevisions(revisions: Record<string, number>): Promise<void> {
  await getPlatform().storage.set(StorageKeys.VAULT_BUCKET_REVISIONS, revisions);
}
