...
import { replaceManifestRevisions } from './ManifestRevisions';
...
export async function openLegacySqliteBlobSnapshot(snapshot: LegacySqliteBlobSnapshot): Promise<LegacySqliteBlobPassthrough> {
  ...
  await getPlatform().storage.remove(StorageKeys.VAULT_CONTENT_FINGERPRINTS);
  ...
  if (snapshot.personalManifestId) {
    await getPlatform().storage.set(StorageKeys.VAULT_PERSONAL_MANIFEST_ID, snapshot.personalManifestId);
