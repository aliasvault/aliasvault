...
import { dirtyScopeStorageKey, StorageKeys } from '../constants/StorageKeys';
import { getPlatform } from '../platform/ClientPlatform';

import { ALL_VAULT_MUTATION_SCOPES, hasUserVisibleScope, type VaultMutationScope } from './VaultMutationScope';
...
export async function getDirtyScopes(): Promise<VaultMutationScope[]> {
  const flags = await Promise.all(
    ALL_VAULT_MUTATION_SCOPES.map(async scope => ((await getPlatform().storage.get(dirtyScopeStorageKey(scope))) === true ? scope : null))
  );
...
export async function clearDirtyScopes(): Promise<void> {
  await getPlatform().storage.removeMany(ALL_VAULT_MUTATION_SCOPES.map(scope => dirtyScopeStorageKey(scope)));
}
...
export async function hasUnsyncedUserChanges(): Promise<boolean> {
  if (await getPlatform().storage.get(StorageKeys.IS_DIRTY) !== true) {
    return false;
  }
