...
setPlatform(createInMemoryPlatform({
  loadRustCoreWasm: async (): Promise<BufferSource> => readFileSync(path.join(import.meta.dirname, 'wasm/aliasvault_core_bg.wasm')),
  locateSqlJsFile: (file: string): string => path.join(path.dirname(require.resolve('sql.js')), file),
}));
