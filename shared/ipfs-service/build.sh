...
# Define output targets for ipfs-service
TARGETS=(
  "../../apps/browser-extension/src/utils/dist/shared/ipfs-service"
)

# Build and distribute ipfs-service
package_name="ipfs-service"
package_path="."
...
To make changes:
1. Update the source files in the \`/shared/ipfs-service/src\` directory
2. Run the \`build.sh\` script in the module directory to regenerate the outputs and copy them here.
EOF
done

echo "✅ IPFS service build and copy completed."
